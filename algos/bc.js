...
export function createDrawing(ctx, options){
    ...
    let gradientType = Math.floor(prng.rand()*2);
    gradientType = 0;
    gradientVisibility = .5;
    gradientVisibility = 0;
    ...
    let slicedPalette;
    if(palette.length < 11118)
        slicedPalette = palette;
    else
        slicedPalette = palette.slice(startIndex, startIndex + ncolors);
